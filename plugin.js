...
function YouTubeFloat() {
  ...
  return jsxs('div', { className: 'relative flex h-full min-h-0 flex-col bg-black/20', children: [
    videoId ? jsx('webview', { allowpopups: 'true', className: 'min-h-0 flex-1 bg-black', partition: 'persist:hermes-youtube-float-player', ref: playerRef, src: watchSrc }) : jsx('div', { className: 'grid min-h-0 flex-1 place-items-center bg-black text-xs text-white/60', children: `${VERSION}: ${status}` }),
