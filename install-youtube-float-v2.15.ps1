...
$targets | Select-Object -Unique | ForEach-Object {
  ...
  if ($text -notmatch 'v65-search-guard') { throw "Copy verification failed: $_" }
  $written += $_
}
Write-Host 'Installed YouTube Float v65-search-guard to:'
$written | ForEach-Object { Write-Host " - $_" }
Write-Host 'Fully quit Hermes Desktop and reopen it. Pane title should be YouTube v65.'
