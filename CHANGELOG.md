## v3.141
- Harden the player integration API v1 as if a third-party plugin already depends on it. Add a 5s control timeout so a hung player can never leave a consumer hanging (replies {code: player_error, error: "control timed out"}); reject non-object params at the envelope; derive `isShort` from the active queue item and reclassify `isShort`/`isLive` as best-effort (not guaranteed) with the matching doc field-ladder update; align the coerceFiniteNumber contract between src/ and the bridge (blank/whitespace strings are rejected consistently). Contract + parity tests extended to freeze the hardened v1 behaviour.
...
- Keep future version bumps scoped away from old changelog entries.

## v3.118
- Add stdlib release zip builder/checker.
- Run release-zip package verification in CI.
- Document the zip builder in README and release checklist.
