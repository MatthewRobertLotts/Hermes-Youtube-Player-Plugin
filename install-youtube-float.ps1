...
$targets | Select-Object -Unique | ForEach-Object {
  ...
  if ($text -notmatch 'v15-size-presets') { throw "Copy verification failed: $_" }
  $written += $_
}
Write-Host 'Installed YouTube Float v15-size-presets to:'
$written | ForEach-Object { Write-Host " - $_" }
Write-Host 'Fully quit Hermes Desktop and reopen it. Pane title should be YouTube v15.'
