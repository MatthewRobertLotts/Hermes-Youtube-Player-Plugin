...
$targets | Select-Object -Unique | ForEach-Object {
  ...
  if ($text -notmatch 'v62-playlist-retry') { throw "Copy verification failed: $_" }
  $written += $_
}
Write-Host 'Installed YouTube Float v62-playlist-retry to:'
$written | ForEach-Object { Write-Host " - $_" }
Write-Host 'Fully quit Hermes Desktop and reopen it. Pane title should be YouTube v62.'
