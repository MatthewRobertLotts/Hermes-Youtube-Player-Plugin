...
$targets | Select-Object -Unique | ForEach-Object {
  ...
  if ($text -notmatch 'v29-next-prev-fix') { throw "Copy verification failed: $_" }
  $written += $_
}
Write-Host 'Installed YouTube Float v29-next-prev-fix to:'
$written | ForEach-Object { Write-Host " - $_" }
Write-Host 'Fully quit Hermes Desktop and reopen it. Pane title should be YouTube v29.'
