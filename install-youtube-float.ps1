...
$targets | Select-Object -Unique | ForEach-Object {
  ...
  if ($text -notmatch 'v12-v9-layout-fix') { throw "Copy verification failed: $_" }
  $written += $_
}
Write-Host 'Installed YouTube Float v12-v9-layout-fix to:'
$written | ForEach-Object { Write-Host " - $_" }
Write-Host 'Fully quit Hermes Desktop and reopen it. Pane title should be YouTube v12.'
