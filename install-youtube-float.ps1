...
$targets | Select-Object -Unique | ForEach-Object {
  ...
  if ($text -notmatch 'v41-dropdown-stays-open') { throw "Copy verification failed: $_" }
  $written += $_
}
Write-Host 'Installed YouTube Float v41-dropdown-stays-open to:'
$written | ForEach-Object { Write-Host " - $_" }
Write-Host 'Fully quit Hermes Desktop and reopen it. Pane title should be YouTube v41.'
