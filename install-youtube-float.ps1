...
$targets | Select-Object -Unique | ForEach-Object {
  ...
  if ($text -notmatch 'v54-verified-sp-lockup') { throw "Copy verification failed: $_" }
  $written += $_
}
Write-Host 'Installed YouTube Float v54-verified-sp-lockup to:'
$written | ForEach-Object { Write-Host " - $_" }
Write-Host 'Fully quit Hermes Desktop and reopen it. Pane title should be YouTube v54.'
