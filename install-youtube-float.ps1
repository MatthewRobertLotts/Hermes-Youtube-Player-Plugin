...
$targets | Select-Object -Unique | ForEach-Object {
  ...
  if ($text -notmatch 'v63-player-playlist') { throw "Copy verification failed: $_" }
  $written += $_
}
Write-Host 'Installed YouTube Float v63-player-playlist to:'
$written | ForEach-Object { Write-Host " - $_" }
Write-Host 'Fully quit Hermes Desktop and reopen it. Pane title should be YouTube v63.'
