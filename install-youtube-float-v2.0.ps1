...
$targets | Select-Object -Unique | ForEach-Object {
  ...
  if ($text -notmatch 'v50-text-buttons') { throw "Copy verification failed: $_" }
  $written += $_
}
Write-Host 'Installed YouTube Float v50-text-buttons to:'
$written | ForEach-Object { Write-Host " - $_" }
Write-Host 'Fully quit Hermes Desktop and reopen it. Pane title should be YouTube v50.'
