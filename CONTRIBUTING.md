...
sh /tmp/youtube-float/install-youtube-float-v3.123.sh
```

Windows PowerShell:

```powershell
Expand-Archive -Force .\youtube-float-desktop-plugin-v3.123.zip $env:TEMP\youtube-float-v3.123
cd $env:TEMP\youtube-float-v3.123
...
sh -n install-youtube-float-v3.123.sh
```

If available, also run:

```bash
shellcheck install-youtube-float.sh install-youtube-float-v3.123.sh
