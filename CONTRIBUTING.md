...
sh /tmp/youtube-float/install-youtube-float-v3.130.sh
```

Windows PowerShell:

```powershell
Expand-Archive -Force .\youtube-float-desktop-plugin-v3.130.zip $env:TEMP\youtube-float-v3.130
cd $env:TEMP\youtube-float-v3.130
powershell -ExecutionPolicy Bypass -File .\install-youtube-float-v3.130.ps1
```
...
4. Confirm the pane title shows `YouTube v3.130 ★`.
...
sh -n install-youtube-float-v3.130.sh
```

If available, also run:

```bash
shellcheck install-youtube-float.sh install-youtube-float-v3.130.sh
