...
sh /tmp/youtube-float/install-youtube-float-v3.140.sh
```

Windows PowerShell:

```powershell
Expand-Archive -Force .\youtube-float-desktop-plugin-v3.140.zip $env:TEMP\youtube-float-v3.140
cd $env:TEMP\youtube-float-v3.140
powershell -ExecutionPolicy Bypass -File .\install-youtube-float-v3.140.ps1
```
...
4. Confirm the pane title shows `YouTube v3.140 ★`.
...
sh -n install-youtube-float-v3.140.sh
```

If available, also run:

```bash
shellcheck install-youtube-float.sh install-youtube-float-v3.140.sh
