...
sh /tmp/youtube-float/install-youtube-float-v3.125.sh
```

Windows PowerShell:

```powershell
Expand-Archive -Force .\youtube-float-desktop-plugin-v3.125.zip $env:TEMP\youtube-float-v3.125
cd $env:TEMP\youtube-float-v3.125
powershell -ExecutionPolicy Bypass -File .\install-youtube-float-v3.125.ps1
```
...
4. Confirm the pane title shows `YouTube v3.125 ★`.
...
sh -n install-youtube-float-v3.125.sh
```

If available, also run:

```bash
shellcheck install-youtube-float.sh install-youtube-float-v3.125.sh
