...
sh /tmp/youtube-float/install-youtube-float-v3.128.sh
```

Windows PowerShell:

```powershell
Expand-Archive -Force .\youtube-float-desktop-plugin-v3.128.zip $env:TEMP\youtube-float-v3.128
cd $env:TEMP\youtube-float-v3.128
powershell -ExecutionPolicy Bypass -File .\install-youtube-float-v3.128.ps1
```
...
4. Confirm the pane title shows `YouTube v3.128 ★`.
...
sh -n install-youtube-float-v3.128.sh
```

If available, also run:

```bash
shellcheck install-youtube-float.sh install-youtube-float-v3.128.sh
