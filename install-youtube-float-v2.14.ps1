...
$targets | Select-Object -Unique | ForEach-Object {
  ...
  if ($text -notmatch 'v64-entry-play') { throw "Copy verification failed: $_" }
  $written += $_
}
Write-Host 'Installed YouTube Float v64-entry-play to:'
$written | ForEach-Object { Write-Host " - $_" }
Write-Host 'Fully quit Hermes Desktop and reopen it. Pane title should be YouTube v64.'
